...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_ADMIN_USERNAME}" --dbname "$POSTGRES_DB" <<-EOSQL
  drop database if exists ${POSTGRES_TEST_DBNAME};
  create database ${POSTGRES_TEST_DBNAME} with owner ${POSTGRES_ADMIN_USERNAME};

  \c todo_test

  -- 1) データベース接続権限
  grant connect on database ${POSTGRES_TEST_DBNAME} to ${POSTGRES_OPERATIONS_USERNAME};
  ...
  grant select, insert, update, delete, truncate on all tables in schema public to ${POSTGRES_OPERATIONS_USERNAME};
  ...
  alter default privileges for role ${POSTGRES_ADMIN_USERNAME} in schema public
    grant select, insert, update, delete, truncate on tables to ${POSTGRES_OPERATIONS_USERNAME};
  alter default privileges for role ${POSTGRES_ADMIN_USERNAME} in schema public
