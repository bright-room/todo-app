...
このドキュメントは、Todo アプリの開発に携わる開発者向けの基本情報を提供します。
...
3. [プロジェクト構造](#プロジェクト構造)
4. [バックエンド開発ガイドライン](#バックエンド開発ガイドライン)
5. [フロントエンド開発ガイドライン](#フロントエンド開発ガイドライン)
6. [共有モジュール開発ガイドライン](#共有モジュール開発ガイドライン)
7. [データベース開発ガイドライン](#データベース開発ガイドライン)
8. [テスト戦略](#テスト戦略)
9. [ビルドとデプロイメント](#ビルドとデプロイメント)
10. [コード品質とベストプラクティス](#コード品質とベストプラクティス)
...
- **Docker** (アプリケーションのコンテナ化とローカル開発環境)
...
- **スケーラブル**: 3層レイヤードアーキテクチャ+ドメインモデルによる保守性と拡張性に優れた設計
...
基本的には利用しない。
複雑なバリデーションルール等が存在する場合、ドメインモデルを簡潔に保つためバリデーションルールの実装のみを個別に切り出す。

#### presentation

ユーザーからの入力（HTTPリクエスト等）を受け取り、アプリケーションサービスに橋渡しをする層 。
